...
* BIMモデル作成時
  ...
  * 窓やドアの開口部分は"ボイド"を配置して作成する。開口部分は、"ボイド"と"開口部"の2つの方法で作成することができますが、"開口部"で作成した場合、その開口部につく窓やドアが欠落や欠損する場合があります。
  * 外壁と内壁を連続した壁として作成しない。正しく、外壁を判定できない場合があります。
  * スラブ・床は屋内外で作り分ける。屋内外で連続したスラブの場合、屋外スラブを判定できない場合があります。
 
* BIMモデルからIFC出力時
  * CityGMLに変換する必要のない要素は出力対象としない。
  * 屋上などの屋外空間に配置された部屋（Revit）やゾーン（ArchiCAD）は出力対象としない。
  * 部屋境界（Revit）情報を出力対象としない。
